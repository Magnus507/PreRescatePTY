  ...
  if coalesce((v_visibility->>'contacts')::boolean, true) then
    select coalesce(
      jsonb_agg(
        ...
        order by pc.priority, c.created_at
      ),
      '[]'::jsonb
    )
      into v_contacts
    from public.v2_profile_emergency_contacts pc
    join public.v2_emergency_contacts c on c.id = pc.contact_id
    where pc.profile_id = v_profile.id
      and pc.show_publicly = true
      and c.account_id = v_assignment.account_id
      and c.status = 'active';
  end if;
