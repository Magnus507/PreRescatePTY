        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           ...
           <div className="lg:col-span-1 space-y-6">
              <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm">
                 ...
                 {/* Chip Management actions */}
                 <div className="mt-8 pt-8 border-t border-slate-50 space-y-4">
                    <div className="p-5 bg-emerald-50 rounded-[2rem] border border-emerald-100 shadow-inner">
                       <p className="text-[10px] font-black uppercase tracking-widest text-emerald-800/40 mb-3">Vinculación por Código</p>
                       <form onSubmit={onAssignChip} className="flex gap-2">
                          <input 
                            placeholder="ShortCode / Serial" 
                            className="flex-1 bg-white border border-emerald-100 rounded-xl px-4 py-2.5 text-xs font-bold focus:ring-2 focus:ring-emerald-500/20 outline-none"
                            value={assignShortCode}
                            onChange={(e) => setAssignShortCode(e.target.value)}
                            required
                          />
                          <button type="submit" className="px-4 py-2.5 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase shadow-lg shadow-emerald-200 hover:scale-105 active:scale-95 transition-all">OK</button>
                       </form>
                    </div>

                    <div className="p-5 bg-slate-50 rounded-[2rem] border border-slate-100 shadow-inner">
                       <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">Asignación Masiva (Lote)</p>
                       <form onSubmit={onBulkAssign} className="flex items-center gap-2">
                          <input 
                            type="number"
                            min={1}
                            className="w-16 bg-white border border-slate-200 rounded-xl px-2 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-primary/20"
                            value={bulkAssignCount}
                            onChange={(e) => setBulkAssignCount(Number(e.target.value))}
                          />
                          <button type="submit" className="flex-1 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-700 transition-all shadow-md active:scale-95">Transferir</button>
                       </form>
                    </div>
                 </div>
              </div>
