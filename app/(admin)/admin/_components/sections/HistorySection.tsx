...
export function HistorySection() {
  ...
  const [submitted, setSubmitted] = useState<{ search: string; entityType: string }>({ search: "", entityType: "" });
  const [data, setData] = useState<HistoryResponse | null>(null);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    const run = async () => {
      if (!submitted.search) return;
      setLoading(true);
      try {
        const params = new URLSearchParams();
        params.set("search", submitted.search);
        if (submitted.entityType) params.set("entityType", submitted.entityType);
        const res = await fetch(`/api/admin/operations/history?${params.toString()}`, { cache: "no-store" });
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
          <label className="flex-1">
            <span className="mb-2 block text-[10px] font-black uppercase tracking-widest text-slate-400">Buscar</span>
          ...
          <button
            type="button"
            onClick={() => setSubmitted({ search: search.trim(), entityType })}
          ...
          <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
            {data.suggestions.map((item) => (
              <button
                ...
                type="button"
                onClick={() => setSubmitted({ search: item.label, entityType: item.type })}
