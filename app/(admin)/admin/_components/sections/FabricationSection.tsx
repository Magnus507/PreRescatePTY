    ...
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {materialSummary.chipsNfc > 0 && (
            <div className="rounded-lg border border-indigo-200 bg-indigo-50/50 px-3 py-2.5">
              <p className="text-lg font-black text-indigo-800">{materialSummary.chipsNfc}</p>
              <p className="text-[9px] font-bold uppercase tracking-wider text-indigo-600">Chips NFC</p>
            </div>
          )}
          {materialSummary.pulseras > 0 && (
            <div className="rounded-lg border border-emerald-200 bg-emerald-50/50 px-3 py-2.5">
              <p className="text-lg font-black text-emerald-800">{materialSummary.pulseras}</p>
              <p className="text-[9px] font-bold uppercase tracking-wider text-emerald-600">Pulseras</p>
            </div>
          )}
          {materialSummary.credenciales > 0 && (
            <div className="rounded-lg border border-amber-200 bg-amber-50/50 px-3 py-2.5">
              <p className="text-lg font-black text-amber-800">{materialSummary.credenciales}</p>
              <p className="text-[9px] font-bold uppercase tracking-wider text-amber-600">Credenciales</p>
            </div>
          )}
          {materialSummary.stickers > 0 && (
            <div className="rounded-lg border border-rose-200 bg-rose-50/50 px-3 py-2.5">
              <p className="text-lg font-black text-rose-800">{materialSummary.stickers}</p>
              <p className="text-[9px] font-bold uppercase tracking-wider text-rose-600">Stickers</p>
            </div>
          )}
          {materialSummary.sobresActivacion > 0 && (
            <div className="rounded-lg border border-purple-200 bg-purple-50/50 px-3 py-2.5">
              <p className="text-lg font-black text-purple-800">{materialSummary.sobresActivacion}</p>
              <p className="text-[9px] font-bold uppercase tracking-wider text-purple-600">Sobres activación</p>
            </div>
          )}
          {materialSummary.lanyards > 0 && (
            <div className="rounded-lg border border-sky-200 bg-sky-50/50 px-3 py-2.5">
              <p className="text-lg font-black text-sky-800">{materialSummary.lanyards}</p>
              <p className="text-[9px] font-bold uppercase tracking-wider text-sky-600">Lanyards</p>
            </div>
          )}
          {materialSummary.cajasCorporativas > 0 && (
            <div className="rounded-lg border border-slate-200 bg-slate-50/50 px-3 py-2.5">
              <p className="text-lg font-black text-slate-800">{materialSummary.cajasCorporativas}</p>
              <p className="text-[9px] font-bold uppercase tracking-wider text-slate-600">Cajas corporativas</p>
            </div>
          )}
        </div>
      </section>

      {/* Secciones agrupadas por tipo de producto */}
      {Object.entries(itemsByType).map(([type, typeItems]) => {
        const icon = TYPE_ICONS[type] || <Package className="h-4 w-4" />;
        const label = TYPE_LABELS[type] || type;
        const count = typeItems.reduce((s, i) => s + i.quantity, 0);
        return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded font-bold uppercase text-muted-foreground">
                        {item.productName}
                      </span>
                      <span
                  ...
                  <div className="flex gap-1.5 shrink-0">
                    {item.chipShortCode && (
                      <button
                        onClick={() => copyToClipboard(item.chipShortCode!, "ShortCode")}
                        className="px-2.5 py-1.5 bg-white border border-slate-200 rounded-lg text-[8px] font-bold uppercase tracking-widest hover:bg-slate-50 inline-flex items-center gap-1"
                        title="Copiar shortCode"
                      >
                        <Copy className="h-2.5 w-2.5" /> Código
                      </button>
                    )}
                    {item.publicLink && (
                      <>
                    ...
                    {item.chipQrUrl && (
                      <button
                        onClick={() => copyToClipboard(item.chipQrUrl!, "QR URL")}
                        className="px-2.5 py-1.5 bg-white border border-slate-200 rounded-lg text-[8px] font-bold uppercase tracking-widest hover:bg-slate-50 inline-flex items-center gap-1"
                        title="Copiar URL del QR"
                      >
                        <QrCode className="h-2.5 w-2.5" /> QR URL
                      </button>
                    )}
                  </div>
                </div>
