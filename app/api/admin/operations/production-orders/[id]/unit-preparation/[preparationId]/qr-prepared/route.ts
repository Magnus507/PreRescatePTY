...
export async function POST(
  ...
  try {
    ...
    await prisma.operationProductionEvent.create({
      data: {
        productionOrderId,
        eventType: "QR_PREPARED",
        quantity: 1,
        reason: "QR preparado para unidad de preparacion",
    ...
    console.error("[operations/production-orders/:id/unit-preparation/:preparationId/qr-prepared] POST error:", error);
    return NextResponse.json({ error: "Error al marcar QR preparado" }, { status: 500 });
  }
}
