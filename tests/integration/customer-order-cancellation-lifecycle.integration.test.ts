...
describe("PostgreSQL integration: PHY-12/13 customer cancellation lifecycle", () => {
  beforeAll(async () => {
    prepareIntegrationEnvironment();
  ...
  it("PHY-12 cancels projection, releases stock, cancels unstarted production and prevents later preparation", async () => {
    const productCode = `${run}-PHY12-SKU`;
    const sourceOrder = await db.order.create({
      data: {
        amount: 50,
        orderNumber: `${run}-PHY12`,
        paymentStatus: "paid",
    ...
    const reservedUnit = await db.operationFinishedGoodUnit.create({
      data: {
        internalLabel: `UNIT-${run}-PHY12-RESERVED`,
        productCode,
        productName: "PHY-12 reserved unit",
    ...
    const production = await db.operationProductionOrder.create({
      data: {
        code: `PROD-${run}-PHY12`.slice(0, 120),
        title: "PHY-12 unstarted backorder",
        status: "planned",
        plannedQuantity: 1,
        producedQuantity: 0,
