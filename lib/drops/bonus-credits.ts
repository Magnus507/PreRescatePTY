import { randomBytes } from "node:crypto";
import { prisma } from "@/lib/prisma";
import {
  BONUS_CREDIT_PATTERN,
  normalizeBonusCreditCode,
} from "@/lib/drops/rules";

function makeBonusEntryCode() {
  return `BE-${randomBytes(8).toString("hex").toUpperCase()}`;
}

export async function redeemBonusCredit(userId: string, rawCode: string) {
  const code = normalizeBonusCreditCode(rawCode);
  if (!BONUS_CREDIT_PATTERN.test(code)) {
    throw new Error("BONUS_CREDIT_INVALID");
  }

  return prisma.$transaction(async (tx) => {
    const rows = await tx.$queryRaw<
      Array<{
        id: string;
        dropId: string | null;
        campaign: string;
        creditAmount: number;
        createdByUserId: string;
        claimedByUserId: string | null;
        claimedAt: Date | null;
        revokedAt: Date | null;
      }>
    >`
      SELECT "id", "dropId", "campaign", "creditAmount", "createdByUserId",
             "claimedByUserId", "claimedAt", "revokedAt"
      FROM "DropBonusCredit"
      WHERE "code" = ${code}
      FOR UPDATE
    `;

    const credit = rows[0];
    if (!credit) throw new Error("BONUS_CREDIT_NOT_FOUND");
    if (credit.revokedAt) throw new Error("BONUS_CREDIT_REVOKED");
    if (credit.claimedAt) throw new Error("BONUS_CREDIT_ALREADY_CLAIMED");

    if (credit.dropId) {
      const drops = await tx.$queryRaw<
        Array<{ id: string; status: string; title: string; archivedAt: Date | null }>
      >`
        SELECT "id", "status"::text AS "status", "title", "archivedAt"
        FROM "Drop"
        WHERE "id" = ${credit.dropId}
        FOR UPDATE
      `;
      const drop = drops[0];
      if (!drop) throw new Error("DROP_NOT_FOUND");
      if (drop.status !== "active" || drop.archivedAt) {
        throw new Error("BONUS_CREDIT_DROP_CLOSED");
      }

      const entry = await tx.dropBonusEntry.create({
        data: {
          code: makeBonusEntryCode(),
          dropId: credit.dropId,
          userId,
          reason: `Bonus Credit · ${credit.campaign}`,
          createdByUserId: credit.createdByUserId,
          sourceBonusCreditId: credit.id,
        },
        select: { id: true, code: true, reason: true, createdAt: true },
      });

      await tx.dropBonusCredit.update({
        where: { id: credit.id },
        data: {
          claimedByUserId: userId,
          claimedAt: new Date(),
        },
      });

      return {
        kind: "drop_entry" as const,
        entry,
        drop: { id: drop.id, title: drop.title },
        creditAmount: 0,
      };
    }

    await tx.$executeRaw`
      SELECT pg_advisory_xact_lock(hashtext(${`reward-credit:${userId}`}))
    `;

    await tx.rewardCreditLedger.create({
      data: {
        userId,
        amount: credit.creditAmount,
        sourceKey: `bonus-code:${credit.id}`,
        sourceType: "admin",
        sourceId: credit.id,
        description: `Bonus Credit general · ${credit.campaign}`,
        createdByUserId: credit.createdByUserId,
      },
    });

    await tx.dropBonusCredit.update({
      where: { id: credit.id },
      data: {
        claimedByUserId: userId,
        claimedAt: new Date(),
      },
    });

    const balance = await tx.rewardCreditLedger.aggregate({
      where: { userId },
      _sum: { amount: true },
    });

    return {
      kind: "wallet_credit" as const,
      entry: null,
      drop: null,
      creditAmount: credit.creditAmount,
      balance: balance._sum.amount ?? 0,
    };
  });
}
