...
- La navegación del panel cliente pasó a mostrar solo `Mis dispositivos` como entrada principal para chips.
- El estado `?activate=true` sigue funcionando para abrir la pestaña de activación dentro de la misma pantalla.
...
## 9. Decisión de navegación

- `Activar chip` deja de ser un módulo independiente en el sidebar.
- La acción vive dentro de `Mis dispositivos` como una pestaña/modo de la misma pantalla.
- Los enlaces existentes a `/dashboard/chips?activate=true` siguen siendo válidos.
- La ruta pública `/activar` puede seguir redirigiendo a esa vista sin romper compatibilidad.
