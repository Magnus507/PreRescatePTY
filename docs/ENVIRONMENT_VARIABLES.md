...
These variables are injected only into the DR GitHub Actions workflows or an explicitly trusted recovery shell. They must not be added to the Vercel application runtime merely for convenience.

| Variable | Sensitive | Purpose |
| --- | --- | --- |
| `DR_SOURCE_DB_URL` | yes | Production database connection used read-only by the logical backup. |
| `DR_SOURCE_SUPABASE_URL` | no | Production Supabase API origin used for Storage export. |
| `DR_SOURCE_SUPABASE_SERVICE_ROLE_KEY` | yes | Backend-only source credential used to export Storage objects. |
| `DR_BACKUP_PASSPHRASE` | yes | High-entropy, 32+ character secret used to encrypt and authenticate every backup package. |
| `DR_TARGET_DB_URL` | yes | Dedicated restore-test database connection; destructive reset and restore are permitted only here. |
| `DR_TARGET_SUPABASE_URL` | no | Dedicated restore-test Supabase API origin. |
| `DR_TARGET_SUPABASE_SERVICE_ROLE_KEY` | yes | Backend-only restore-test credential used to rehydrate Storage. |
| `DR_PRODUCTION_PROJECT_REF` | no | Hard-stop project reference that restore scripts must refuse. |
| `DR_TARGET_PROJECT_REF` | no | Required isolated restore-test project reference. |
| `DR_BACKUP_ID` | no | Per-run identifier generated by the backup workflow. |
| `DR_CONFIRM_ISOLATED_TARGET` | no | Exact `YES` confirmation required by destructive restore-test scripts. |
| `DR_RESTORE_MODE` | no | Selects full or data-only recovery; certification uses data-only against the pre-provisioned isolated schema. |
| `DR_ALLOW_TARGET_OBJECTS` | no | Explicit isolated-target overwrite guard for Storage certification. |
| `DR_SENTINEL_ORDER_NUMBER` | no | Non-secret order identifier used only to prove recovery of a known record. |
